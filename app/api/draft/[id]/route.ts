...
import { generateDraft } from '@/lib/classify';
...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const draft = await generateDraft(await getResolvedSettings(user), lead, track);
    await db.update(leads).set({ draft }).where(and(eq(leads.id, lead.id), eq(leads.ownerId, user)));
